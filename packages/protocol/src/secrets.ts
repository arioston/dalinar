/**
 * Three-layer secret detection for note content.
 *
 * Layer 1 — Known prefix patterns (GitHub PATs, AWS keys, JWTs, etc.)
 * Layer 2 — High-entropy hex/base64 strings
 * Layer 3 — Keyword proximity (credential keywords near high-entropy strings)
 *
 * Canonical implementation shared between Jasnah and Sazed.
 */

import type { SecretDetection } from "./types.js";
...
/**
 * Heuristic: skip strings that look like code identifiers, file paths,
 * or compound names. Real secrets have balanced char distribution.
 */
...
const PREFIX_RULES: ReadonlyArray<{ id: string; pattern: RegExp }> = [
...
/** Scans text for potential secrets. Returns the first detection found, or null. */
export function detectSecrets(text: string): SecretDetection | null {
  // Layer 1: known prefixes
  for (const rule of PREFIX_RULES) {
    const match = rule.pattern.exec(text);
    if (match) {
      return { rule: rule.id, position: match.index, snippet: maskSnippet(text, match.index, match[0].length) };
    }
  }
  ...
  while ((m = hexRe.exec(text)) !== null) {
    if (shannonEntropy(m[0]) > 3.0) {
      return { rule: "high-entropy-hex", position: m.index, snippet: maskSnippet(text, m.index, m[0].length) };
    }
  }
  ...
  while ((m = base64Re.exec(text)) !== null) {
    ...
    if (shannonEntropy(m[0]) > 3.0) {
      return { rule: "high-entropy-base64", position: m.index, snippet: maskSnippet(text, m.index, m[0].length) };
    }
  }
  ...
  while ((m = candidateRe.exec(text)) !== null) {
    ...
    for (const kwRe of KEYWORD_PATTERNS) {
      if (kwRe.test(window)) {
        return { rule: "keyword-proximity", position: m.index, snippet: maskSnippet(text, m.index, m[0].length) };
      }
    }
  }
...
/** Scans title + content of a note. Returns all detections. */
export function detectSecretsInNote(title: string, content: string): ReadonlyArray<SecretDetection> {
  const detections: SecretDetection[] = [];
  const titleDetection = detectSecrets(title);
  if (titleDetection) detections.push({ ...titleDetection, rule: `title:${titleDetection.rule}` });
  const contentDetection = detectSecrets(content);
  if (contentDetection) detections.push(contentDetection);
  return detections;
}
