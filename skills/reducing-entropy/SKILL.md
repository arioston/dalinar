...
- Could a precise type at the boundary eliminate scattered checks throughout the codebase?
...
- **"Better separation of concerns"** - More files with the same logic = more entropy. But if separating concerns lets you *delete* duplicated checks or centralize scattered control flow, count the net. Cosmetic separation adds code. Structural separation that eliminates duplication removes it.
- **"Type safety"** - Distinguish between *naming* and *modeling*. A named wrapper (`newtype Email = Email String`) that just adds ceremony is entropy — it's validation wearing a hat. But a constructive type whose *structure* makes illegal states unrepresentable (a sum type instead of two optional fields, a `NonEmpty` instead of a list + checks) deletes every downstream impossible branch. Count the *total* lines removed, not just the lines added.
